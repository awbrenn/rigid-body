...
Solver::Solver(RigidMesh *Spring_mesh, double time_step, double Ground_level, double Coefficient_of_restitution,
               double Coefficient_of_friction) {
  rigid_mesh = Spring_mesh;
  dt = time_step;
...
void Solver::addExternalForces() {
  ...
  for (auto vp = rigid_mesh->vparticles.begin(); vp < rigid_mesh->vparticles.end(); ++vp) {
    vp->force = gravity * vp->mass;
  }
}

void Solver::addStrutForces() {
  for(auto s = rigid_mesh->struts.begin(); s < rigid_mesh->struts.end(); ++s) {
    VertexParticle* vp1 = &rigid_mesh->vparticles[s->v1];
    VertexParticle* vp2 = &rigid_mesh->vparticles[s->v2];
    ...
    if (s->f1 != UNASSIGNED and s->f2 != UNASSIGNED) {
      ...
      unshared_vertices = rigid_mesh->getUnsharedVertices((unsigned int) s->f1, (unsigned int) s->f2);

      x0 = &rigid_mesh->vparticles[s->v1];
      x1 = &rigid_mesh->vparticles[s->v2];
      x2 = &rigid_mesh->vparticles[unshared_vertices[0]];
      x3 = &rigid_mesh->vparticles[unshared_vertices[1]];
      ...
      face_normal_left = rigid_mesh->calculateFaceNormal((unsigned int) s->f1);
      face_normal_right = rigid_mesh->calculateFaceNormal((unsigned int) s->f2);
...
void Solver::update(unsigned int integrator, Mesh *render_mesh, Vector3d User_acceleration) {
  ...
  rigid_mesh->convertToRenderMesh(render_mesh);
